...
Ns = list(map(int, np.logspace(np.log10(5_000), np.log10(1_000), 21)))
repeats = 10 
...
results = []
for _ in tqdm(range(repeats), position=0, leave=False):
...
plt.suptitle(f"local rbf={rbf}, poly_deg={poly_deg}")
plt.savefig(f"media/local_rbf={rbf}_poly_deg={poly_deg}_k={stencil_size}.png")
